...
sub _build_current_target_file {
    my $self = shift;
    my $now = $self->to || now();
    return $self->home->file( $now->year, sprintf( '%02d', $now->month ),
        'target.json' );
}
has 'targets' => (
    is         => 'ro',
...
sub cmd_overtime {
    ...
    foreach my $project (@$raw_filter) {
        ...
        if ( $project_tree->{$project}{children} ) {    # TODO recurse
            foreach
                my $child ( keys %{ $project_tree->{$project}{children} } )
            {
    ...
    my $s     = \' | ';
    my $table = Text::Table->new( "Project", $s, "Worked", $s, "Target", $s,
        "Status" );
    ...
    while ( my ( $project, $target ) = each %$targets ) {
        ...
        $table->add( $project, $self->beautify_seconds($seconds),
            $target, $status, );
    }
    print $table->title;
    print $table->rule( '-', '+' );
